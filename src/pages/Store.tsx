...
import { productService, InventoryItem, Category, Subcategory } from '@/services/product.service';
...
const Store = () => {
  ...
  const petCategories = useMemo(() => {
    return categories.map(cat => ({
      value: cat.pet_type.toLowerCase() as PetCategory,
      label: cat.pet_type.charAt(0).toUpperCase() + cat.pet_type.slice(1),
      icon: cat.pet_type === 'dogs' ? '🐕' : cat.pet_type === 'cats' ? '🐈' : cat.pet_type === 'birds' ? '🐦' : '🐠',
    }));
  }, [categories]);

  const productCategories = useMemo(() => {
    // Get unique subcategories
    const uniqueSubcats = Array.from(new Set(subcategories.map(s => s.subcategory_name)));
    return uniqueSubcats.map(name => ({
      value: name.toLowerCase() as ProductCategory,
  ...
  }, [products, selectedPet, selectedCategory]);
